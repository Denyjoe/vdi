...
export default function CreateGroupModal({ onClose, onCreated }) {
    ...
    const handleCopyLink = () => {
        if (successData?.invite_code) {
            navigator.clipboard.writeText(`https://app.clouddesk.io/join/group/${successData.invite_code}`);
            setCopiedLink(true);
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
                <div className="bg-[#0D1526] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl p-8 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-slate-400 mb-6">Share this code with members to let them join.</p>
                        ...
                        <div className="flex items-center justify-between bg-[#050B18] p-3 rounded-lg border border-white/5 mb-8">
                            <span className="text-xs text-slate-400 truncate max-w-[200px]">
                                app.clouddesk.io/join/group/{successData.invite_code}
                            </span>
                        ...
                        <button onClick={() => { onClose(); navigate(`/instructor/groups/${successData.id}`); }} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-medium transition-colors shadow-lg shadow-indigo-500/20">
                            Open Group &rarr;
                        </button>
                    </div>
                </div>
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-[#0B1120] border border-white/10 rounded-2xl w-full max-w-xl shadow-2xl flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-white/10 flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-white">Create Group</h2>
                        <p className="text-sm text-slate-400 mt-1">Organize students and manage shared access.</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    <form id="create-group-form" onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-2">Visibility Type</label>
                                <select 
                                    name="group_type"
                        ...
                        <div>
                            ...
                            <div className="w-full bg-white/5 border border-white/10 rounded-xl p-2 flex flex-wrap gap-2 min-h-[52px]">
                                ...
                                <input 
                                    ...
                                    className="flex-1 bg-transparent border-none outline-none text-white text-sm min-w-[120px] px-2 py-1"
                                />
                            </div>
                        </div>
                    </form>
