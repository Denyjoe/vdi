...
const StorageTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-xs shadow-lg">
      ...
      <p className="text-slate-300">Used: {payload[0]?.value} GB</p>
    </div>
  );
};
...
const GaugeCard = ({ title, value, centerLine1, centerLine2, color }) => {
  ...
  return (
    <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-md flex flex-col items-center">
      ...
      <div className="relative w-full" style={{ height: 160 }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-6 pointer-events-none">
          <p className="text-2xl font-bold text-white">{centerLine1}</p>
          {centerLine2 && <p className="text-xs text-slate-400 mt-0.5">{centerLine2}</p>}
        </div>
      </div>
    </div>
  );
...
export default function AdminHardwarePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── 1. Server Status Bar ──────────────────────────────────────── */}
      <div className="bg-slate-800 border border-slate-700 rounded-xl px-6 py-4 flex flex-wrap items-center gap-4 text-sm shadow-md">
        ...
        <span className="text-slate-400">Proxmox VE {stats.proxmox_version}</span>

        <span className="hidden sm:block h-4 w-px bg-slate-600" />
        <span className="flex items-center gap-1.5 text-slate-400">
          <Server className="w-3.5 h-3.5" />
          Uptime: <span className="text-white">{stats.uptime_days} days</span>
        </span>
        ...
        <span className="flex items-center gap-1.5 text-slate-400">
          ...
          <span className="text-white">{vmSum.total}</span> total VMs
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Network I/O */}
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-md flex flex-col justify-center gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-emerald-400">
                ...
                <span className="text-slate-300 text-sm">OUT</span>
              </div>
              <span className="text-white font-bold text-lg">
        ...
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-md">
          ...
          {pieData.length === 0 ? (
            <div className="h-64 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-slate-400 text-sm">No VMs provisioned yet</p>
            </div>
          ) : (
            <>
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-700">
          <h3 className="text-base font-semibold text-white">Storage Pools</h3>
        </div>
          ...
          <table className="w-full text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {stats.storage_pools.map((pool) => {
                ...
                return (
                  <tr key={pool.name} className="hover:bg-slate-700/20 transition-colors">
                    <td className="px-6 py-4 font-medium text-white">{pool.name}</td>
                    <td className="px-6 py-4">
                      <span className="bg-slate-700 border border-slate-600 px-2 py-0.5 rounded text-xs">
                        {pool.type}
                      </span>
                    </td>
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-700">
          <h3 className="text-base font-semibold text-white">Proxmox Nodes</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-slate-300">
            <thead className="text-xs uppercase text-slate-400 bg-slate-900/50 border-b border-slate-700">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {stats.nodes.map((n) => {
                ...
                return (
                  <tr key={n.name} className="hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right text-white font-medium">{n.vm_count}</td>
                    <td className="px-6 py-4">
